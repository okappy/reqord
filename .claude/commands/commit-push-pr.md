...
引数に `#N` が指定されていない場合でも、ブランチ名からissue番号を自動推定する:
- `feature/147-xxx`, `fix/123-xxx`, `issue-42` などのブランチ名 → issue #147, #123, #42 を推定
- 推定した番号でissueが存在し、OPENであればPR本文に `Closes #N` を自動挿入
- **これにより、マージ時にissueが自動クローズされる**
